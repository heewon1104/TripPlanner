...
function ExpenseCalculationAndSupplies() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${serverHost}:${serverPort}/api/expense`,
          {
            params: {
  ...
  const fetchSuppliesData = async () => {
    try {
      const response = await fetch(
        `${serverHost}:${serverPort}/api/supplies?user_id=${user_id}`
      );
      if (!response.ok) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${serverHost}:${serverPort}/api/supplies`,
          {
            params: {
              user_id: user_id,
            },
  ...
  return (
    <div className={styles["All"] + " " + styles["align-center"]}>
      ...
      <div className={styles["main-set"]}>
        <div
          className={styles["calculator-set"] + " " + styles["border-right"]}
        >
          <div className={styles["expenseCalculatorTitle"]}>
            플래너1 경비 계산
          </div>
          <div className={styles["expenseCalculatoradddiv"]}>
            <button
              className={styles["expenseCalculatoraddbtn"]}
          ...
          <div className={styles["expenseCalculator"]}>
            {expenseData.map((item, index) => (
              <div className={styles["expense-item"]} key={index}>
                ...
                <div className={styles["expense-item-amount"]}>
                  {item.budget}원
                </div>
                <button onClick={() => openExpenseModalForEdit(index)}>
                  수정
                </button>
                <button onClick={() => handleDeleteExpense(index)}>삭제</button>{" "}
                {/* 삭제 버튼 추가 */}
              </div>
            ))}
            <div className={styles["expense-item"]}>
              ...
              <div className={styles["expense-item-amount"]}>
                {expenseData.reduce(
                  (total, item) => total + parseInt(item.budget),
                  0
                )}{" "}
                원
              </div>
        ...
        <div className={styles["calculator-set"]}>
          <div
            className={
              styles["align-center"] + " " + styles["expenseCalculatorTitle"]
            }
          >
            플래너1 준비물
          </div>
          <div
            className={
              styles["align-center"] + " " + styles["expenseCalculatoradddiv"]
            }
          >
            <button
          ...
          <div className={styles["expenseCalculator"]}>
            {suppliesData.map((item, index) => (
              <div className={styles["expense-item"]} key={index}>
                ...
                <button onClick={() => openSuppliesModalForEdit(index)}>
                  수정
                </button>
                <button onClick={() => handleDeleteSupplies(index)}>
                  삭제
                </button>{" "}
                {/* 삭제 버튼 추가 */}
              </div>
